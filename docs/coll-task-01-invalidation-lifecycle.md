...
- [ ] `start` twice creates one subscription
- [ ] `stop` unsubscribes; a second `stop` is a no-op
- [ ] `start` → `stop` → `start` works
- [ ] `stop` during a draining connection does not raise
- [ ] The SDK's local guard is gone and its `close()` tears the subscription down
- [ ] `./scripts/test.sh core` green
- [ ] `./scripts/typecheck.sh` and `./scripts/lint.sh` clean
