...
**2026-08-14 — two rows moved, and neither moved because someone reconsidered
it.** `§10.10` and `SR-M1` are updated below from a production outage on
cobalt-dev: an additive optional on an `extra="forbid"` envelope refused every
call from a lagging pod for three days, which is the failure D13 predicted in
the abstract and had never been charged for. Recorded here rather than only in
the changelog because both rows carry a *recommendation* that the incident
tests: §10.10's rollout order was necessary and insufficient, and SR-M1's
deferral is a live cost rather than a deferred hypothetical.
...
| SR-M1 | Versioning and compatibility promise | Lockstep already covers the in-family Python API; rule the **wire** contract before the first pod-resident deployment | **OPEN, and correctly so — but no longer cost-free, 2026-08-14.** It is Gate C's, which fires when the first pod-resident search deploys, and D13's ignore-unknown flip is the piece it must carry. That flip has now been priced by something other than argument: three days of total refusal on cobalt-dev, a mixed-version fleet turning one additive field into a fleet-wide outage (see §10.10). An ignore-unknown receiver would have degraded instead. The 0.24.3 sender-side pruning mitigates and does not decide |
...
| §10.10 | A per-call deadline on the tool-call envelope | An ask on `agent-tools`; blocks SR-G2 pod-resident | **Half discharged — and its reasoning corrected by an outage, 2026-08-13.** [#317](https://github.com/pacepace/3tears/pull/317) shipped the *accepting* half only, on the reading that the field stays off the wire until a caller populates it. It does not: every *declared* field is serialized, so `deadline_seconds` travelled as an explicit `null` from the day it was declared, and `extra="forbid"` refuses an unknown null exactly as it refuses an unknown value. A 0.23.11 pod refused **every** call from a 0.24.1 registry for three days, on a field no caller had been taught to send. 0.24.3 prunes unset top-level optionals from the registry→pod envelope, which makes the receiver-first order true rather than merely asserted. The sending half still awaits a second cycle, now against a **0.24.3+ fleet floor** rather than a 0.24.1 one. **2026-08-14 — the proxy→pod sending half is built:** `ProxyCallRequest` grows `deadline_seconds` (hop one's accepting half) and the proxy forwards it onto `CallRequest`, clamped to its own wait, so a caller can shorten a call but never buy more time than the tool allows. A caller that declares nothing produces byte-identical bytes to before — pinned against a hand-written reader predating the field, not by asserting a value was passed. What remains is hop one's *sender*: teaching an agent to populate it, which needs the fleet to carry a registry that accepts it, and which must prune unset optionals the same way |
...
| **§5.5** | `skill_eligible`, and whether search stays in the `web` group alias | Both are ACL-visible surface; decide before a per-carrier tool split makes `web` mean something new | **OPEN — and no longer gating, 2026-08-14.** It was recorded as gating on the reading that success check 14 needs the reach turned on. It does not: the face flags govern reach, nothing in 3tears reads them (the surfaces that act on them are hub-side), and check 14 is about *rendering*. The check was split — both in-repo renderings verified against each other, the hub-rendered API face verified where it is exposed with checks 1, 2, 3, 9, 10 — and Gate B closed on that. `skill_eligible` and the `web` alias stay open **on their own merits**, as reach decisions to take before a per-carrier split makes `web` mean something new |
