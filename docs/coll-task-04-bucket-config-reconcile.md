# coll-task-04: Bucket Config Reconcile (`allow_direct`)
...
construction sites. **The defect is real in source and has not been demonstrated
live.** Prove it by opening an existing bucket with a differing requested config
and observing the bind — do not cite the TTL.
...
| KVC-08 | `deny_purge: true` on the collections stream | P1 |
| KVC-09 | The retired `NatsKvClient` is deleted | P1 |
...
- [ ] `ensure_kv_bucket` exists, builds the full KV stream shape, and shares the bucket cache
- [ ] Opening an existing bucket with a differing `direct` raises `KvConfigMismatch`
- [ ] That type is not caught by the L2 accessors, and is raised at startup
- [ ] `js.key_value()` still binds after a reconcile
- [ ] `create_if_missing`'s default is unchanged, and it is a `configure()` flag — no literal in `base.py`
- [ ] KVC-10's reconnect hook is wired and proven by killing NATS and confirming the bucket returns without a hub restart
- [ ] No static-user process can create the collections bucket
- [ ] Hub lifespan declares the collections bucket
- [ ] `NatsKvClient` is gone; no import or patch target remains in any repo
- [ ] Live bucket reports `allow_direct: true`
- [ ] `./scripts/check-all.sh` and `./scripts/test-integration.sh` green; SDK unit suite green
...
- [ ] **No production code opens a KV bucket outside the reconcile primitive.** Suggested: an AST walker over `create_key_value` / `key_value` call sites outside `NatsClient`, mirroring the sanctioned-factory shape the cache-primitive rule already uses.
- [ ] **`KvConfigMismatch` never appears in an `except` clause that also handles `KvError`.** The specific way this fix gets silently undone.
