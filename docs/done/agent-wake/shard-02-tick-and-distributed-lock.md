...
> **DELEGATED (S-2):** the cross-pod tick **pump** described below — lock acquire/degrade-open, due-scan, optimistic-CAS claim, per-fire isolation, drift recording — and the `_compute_next_fire_at` math now live ONCE in the generic `threetears.scheduled_jobs` core (`scheduled_tick_job` + `compute_next_fire_at`). `threetears.agent.wake.tick` is now a thin adapter: `wake_tick_job(pool, nats_client, dispatch_callback)` (signature **unchanged**) builds adapter `ScheduleStore`/`FireStore` over the unchanged wake collections + an adapter dispatch callback, and delegates under the preserved `"agent_wake_tick"` lock key. The wake-specific contract is unchanged (schedule/fire schema, the richer `FireStatus`, webhook/`[SILENT]` handling); the tick's fire/drift/tick-duration metrics moved to the `scheduled_jobs_*` family (the yield-duration histogram is re-emitted by the adapter). The pump internals below are retained for history — the live source of truth is `packages/scheduled-jobs/`.
