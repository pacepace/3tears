...
| MEM-05 | `MemoriesCollection`, `MediaCollection`, `MediaContentCollection`, `MemoryChunkCollection`, `MemoryRefsCollection` all declare `partition_column='agent_id'` in their `TableSchema`. The collections-task-02 framework's `__init_subclass__` enforcement fires; every public method must accept `agent_id` as a required first non-self arg, OR be `@spans_partitions`-decorated, OR be in `partition_exempt_methods`. | P0 |
