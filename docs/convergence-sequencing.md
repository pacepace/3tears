...
(`search-spec.md`), its enablers, and the eval extraction
(`family-convergence.md` §4.2). Detailed in-repo sequencing for the search
build stays in `search-spec.md` §7; this document is the layer above it — who
can start what, and when, across five repos.
**Not sequenced here:** the rest of the convergence program (chat-kit build
§4.11, tiled images §4.12, scrape consolidation §4.13, the observe/models/iam
adoptions) — except where an item below explicitly gates one of them.
...
- **3tears:** build the search leaf — contracts, SearXNG + Tavily adapters,
  ...
  `test_no_bespoke_reuse` widening. → `search-spec.md` §7 Phase 1
- **3tears:** add the three `media-contracts` facet fields (rights status,
  pixel dimensions, direct-file vs containing-page). → `search-spec.md` §4.1
- **discodon:** adopt Python 3.14 (interpreter switch + verify; its floor is
  already `>=3.12`). → `family-convergence.md` open question 1
- **discodon:** land the in-flight character-eval / cassette-delivery work.
  → its own branch; seam rules in `search-spec.md` D28
- **discodon:** carve the eval **StorageProtocol**, migrate `EvalStorage` and
  ...
  store-shape rule. → `family-convergence.md` §4.2 (verified corrections),
  open question 22
- **discodon:** reshape budget refusal to the
  `check(estimate)`/`record(spend)` port shape, copied from
  `threetears.search.contracts`, not invented. → `search-spec.md` §7 Phase 5;
  `family-convergence.md` §4.2
- **metallm:** close the family version lag. → `family-convergence.md` §5
  (metallm)
...
- **3tears:** search Phase 2 — Extract's web path; gut
  ...
  asks. → `search-spec.md` §7 Phase 2, §4
- **3tears:** search Phase 3 — replay, aggregate, select; the replay record
  ...
  storage port (why Phase 1 precedes this). → `search-spec.md` §7 Phase 3,
  §3.10, D26–D28
- **3tears (design only):** decide the stream-protocol channel for
  ...
  *frontend* convergence later, not this sequence's next phase, but deciding
  ...
  → `family-convergence.md` §5 (3tears obligations)
...
- **metallm:** `feature/new-search` — pin the released family, delete both
  side-steps (check 1). → `search-spec.md` §7 Phase 5
...
- **3tears + discodon:** extract `3tears-eval-contracts` first (identity,
  measures, spend/replay composition per D27/D28), then `run` and
  `analysis`; the generator ships package prompts plus the narrow completion
  protocol satisfied by `3tears-models`, and may extract last.
  → `family-convergence.md` §4.2 (incl. verified corrections), open
  questions 2, 3, 22
- **discodon:** consume its own extraction — first consumer, green before
  anyone else binds. → `family-convergence.md` §5 (discodon)

**Checkpoint:** discodon running on the extracted packages; the eval
contracts ratified per SR-M3's per-repo-acceptance pattern.
...
- **metallm:** adopt `eval-*` — today it has zero eval infrastructure, the
  family's biggest gap. → `family-convergence.md` §5
- **scriob:** adopt `eval-{contracts,analysis}` for its continuity corpus.
  → `family-convergence.md` §5
- **samsung:** adopt `eval-run` for its MCP-driver eval.
  → `family-convergence.md` §5
- **hallucinote:** programmatic runner over its scenario briefs and canonical
  verdicts. → `family-convergence.md` §5

**Checkpoint:** the eval workstream of `family-convergence.md` §4.2 is
delivered; the remaining §4 workstreams sequence on their own.
