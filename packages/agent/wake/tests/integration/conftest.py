"""Integration-test fixtures for agent-wake.

Mirrors :mod:`packages.agent.skills.tests.integration.conftest`: the
...
The wake package does not exercise pgvector, so the default
...
async def pg_schema(pg_url: str) -> AsyncIterator[tuple[str, str]]:
    ...
    :rtype: tuple[str, str]
    """
    schema = f"wk_it_{id(object())}".lower().replace("-", "_")
