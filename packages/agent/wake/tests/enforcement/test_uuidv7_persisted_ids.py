"""Package-side enforcement: no ``uuid4`` in agent-wake production source.
...
cursor-paged queries on ``agent_wake_schedules`` / ``wake_fires`` /
``webhook_subscriptions`` rely on the byte-lex ordering matching
creation time. Mixing in uuid4 would silently poison that ordering.
"""
...
_SRC_ROOT = Path(__file__).resolve().parent.parent.parent / "src" / "threetears" / "agent" / "wake"
...
def _collect_source_files() -> list[Path]:
    ...
    backfill in future shards) -- they are exempt from the UUIDv7
    walker, matching the workspace convention.
    """
...
class TestNoUuid4InWakePackage:
    """Pin the UUIDv7 invariant inside the agent-wake package."""
