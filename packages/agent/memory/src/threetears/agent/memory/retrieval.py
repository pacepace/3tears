...
from threetears.langgraph.fence import mint_nonce, untrusted_fence, untrusted_rule
...
def _format_memory_context(
    ...
    if memories:
        ...
        lines.append(untrusted_fence(tag, "\n".join(items)))
    ...
    if deduped_chunks:
        ...
        # chunk_query=...) tool calls. Never auto-render
        # ``chunk.content`` regardless of score -- a high-score chunk
        # surfacing its full transcript would blow the prompt budget
        # on every turn.
        #
        # Hard cap on count too (MAX_SURFACED_CHUNKS) so a query that
        # matches dozens of chunks doesn't flood the headline area.
        ...
        for chunk in surfaced_chunks:
            ...
            # content-prefix when summary is missing (transcript chunks
            # generated by older conversation_summarize calls may not
            # carry one). Never expose more than MAX_CHUNK_SUMMARY_CHARS.
            headline = chunk.get("summary") or chunk.get("content", "")
            if len(headline) > _MAX_CHUNK_SUMMARY_CHARS:
                headline = headline[: _MAX_CHUNK_SUMMARY_CHARS - 3] + "..."
            # Document-chunk metadata (title / page / heading) still
            # surfaces when present so the agent can recognize the
            # source; transcript chunks (no title) just render the
            # chunk_id + headline + recall affordance.
...
# Pull-not-push caps (Shard D D-02). Live here next to
# ``_format_memory_context`` so anyone tuning the retrieval-rendering
# headline budget changes one place.
_MAX_SURFACED_CHUNKS = 3
_MAX_CHUNK_SUMMARY_CHARS = 150
