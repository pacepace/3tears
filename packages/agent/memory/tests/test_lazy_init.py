"""lazy-surface consistency tests for the agent-memory package __init__.
...
import threetears.agent.memory as memory
...
class TestLazySurfaceConsistency:
    def test_all_is_subset_of_lazy(self) -> None:
        assert set(memory.__all__) <= set(memory._LAZY)

    def test_type_checking_block_matches_lazy(self) -> None:
        init_path = Path(memory.__file__)
        assert _type_checking_names(init_path) == set(memory._LAZY)
    ...
    def test_unknown_attribute_raises(self) -> None:
        try:
            memory.definitely_not_an_attribute
...
class TestImportCost:
    def test_types_import_does_not_load_data_stack(self) -> None:
        ...
        probe = (
            "import json, sys\n"
            "import threetears.agent.memory.types\n"
