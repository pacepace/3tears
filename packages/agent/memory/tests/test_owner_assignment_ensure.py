"""``ensure_memory_owner_assignment`` against arity-enforcing collections.

Every other test of this function builds its collections out of duck-typed
stand-ins whose ``get`` accepts one argument of any shape. ``groups`` is keyed
on the composite ``(row_scope, group_id)`` and ``group_members`` on
``(group_id, id)``, so a stand-in like that cannot tell a correct call from one
that passes the bare id -- and for as long as it was the only coverage, both
calls passed the bare id and the function raised ``primary key arity mismatch``
at its first statement, in every process that reached it.
...
@pytest.fixture
def wiring() -> tuple[MemoryAuthorizerDependencies, dict[str, AsyncMock], UUID, UUID, UUID]:
    ...
    :rtype: tuple[MemoryAuthorizerDependencies, dict[str, AsyncMock], UUID, UUID, UUID]
    """
    ...
    deps = MemoryAuthorizerDependencies(
        ...
        role_collection=_BuiltinRoleCollection(
            Role(id=uuid7(), name=MEMORY_OWNER_ROLE_NAME, permissions={}, is_built_in=True),
        ),
...
async def test_ensure_addresses_groups_by_its_composite_key(
    wiring: tuple[MemoryAuthorizerDependencies, dict[str, AsyncMock], UUID, UUID, UUID],
) -> None:
    """the ``groups`` read binds ``row_scope`` alongside ``group_id``.

    :param wiring: bundle / pools / ids fixture
    :ptype wiring: tuple[MemoryAuthorizerDependencies, dict[str, AsyncMock], UUID, UUID, UUID]
    :return: nothing
    :rtype: None
    """
