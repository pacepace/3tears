...
pins the three-way agreement between ``__all__``, the ``_LAZY`` map, and
...
class TestLazySurfaceConsistency:
    def test_all_is_subset_of_lazy(self) -> None:
        assert set(identity.__all__) <= set(identity._LAZY)

    def test_type_checking_block_matches_lazy(self) -> None:
        init_path = Path(identity.__file__)
        assert _type_checking_names(init_path) == set(identity._LAZY)
