...
class TestGroupGetByName:
    """``GroupCollection.get_by_name`` is the exact per-scope group lookup.

    ``name`` is unique per scope in the platform DDL (a partial unique index per
    customer, another across platform-scoped rows), which is what makes this a
    lookup rather than a search. It replaced ``get_by_managed_key``, whose column
    re-declared that same uniqueness under a name that read as something an
    operator had to mint.
    """

    @pytest.mark.asyncio
    async def test_resolves_customer_scoped_group(self) -> None:
        """a customer-scoped group resolves by ``(name, customer_id)``."""
        cid = uuid7()
        ...
        assert pool.fetchrow.await_args.args[1] == "Tenant administrators"
        assert pool.fetchrow.await_args.args[2] == cid

    @pytest.mark.asyncio
    async def test_resolves_platform_scoped_group_with_none_customer(self) -> None:
        """a platform group resolves with ``customer_id=None`` (NULL partition).

        ``IS NOT DISTINCT FROM`` rather than ``=`` is what makes this match: a
        plain equality against NULL matches nothing.
        """
        row = _group_row(customer_id=None, name="Platform administrators")
