"""pytest-friendly orchestration for logger-coverage enforcement.

a single :func:`run_logger_coverage_enforcement` entry point lets
each consumer's thin shell invoke the walker. the runner is the
policy point: it resolves src roots, applies file-level exemptions
...
def run_logger_coverage_enforcement(
    ...
    (which walks path-deps so a missing logger in a sibling package
    is visible to the AST scanner).

    file-level exemptions live in
    :attr:`LoggerCoverageConfig.exempt_files` and are applied inside
    the walker (a listed file is skipped before AST parsing).
    additionally, when
    :attr:`LoggerCoverageConfig.exemptions_path` is set and exists,
    ...
    report = emit_report(
        ...
        mode,
        config.repo_root,
        domain=f"logger_coverage.{walker}",
    )
