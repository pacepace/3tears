"""pytest-friendly orchestration for no-stdlib-logging enforcement.

a single :func:`run_no_stdlib_logging_enforcement` entry point lets
each consumer's thin shell invoke the walker. the runner is the
policy point: it resolves src roots, applies file/line exemptions
inside the walker plus parsed-exemption-file filtering, emits the
...
__all__ = ["run_no_stdlib_logging_enforcement"]
...
def run_no_stdlib_logging_enforcement(
    ...
    src roots come from :attr:`NoStdlibLoggingConfig.src_roots` when
    set, else from :func:`discover_src_roots
    <threetears.enforcement.common.pyproject_discovery.discover_src_roots>`
    (which walks path-deps so a stdlib-logging import in a sibling
    package is visible to the AST scanner).

    file-level exemptions live in
    :attr:`NoStdlibLoggingConfig.exempt_files` and are applied inside
    the walker (a listed file is skipped before AST parsing).
    additionally, when
    :attr:`NoStdlibLoggingConfig.exemptions_path` is set and exists,
    that file is parsed via
    ...
    :ptype config: NoStdlibLoggingConfig
    ...
    report = emit_report(
        ...
        mode,
        config.repo_root,
        domain=f"no_stdlib_logging.{walker}",
    )
