"""tests for the conversation authorize helper (namespace-task-01 phase 10).

mirrors the memory-package authorize tests: builds the bundle against
in-memory Collection stand-ins and the real ACL loaders; exercises the
namespace-materialization happy path, the create-failed denial path,
the owner short-circuit, and the reader / owner assignment paths.
"""
...
class _StubNamespaceEntity:
    ...
    shape used by :func:`_resolve_or_create_conversation_namespace`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        :param data: field data dict (production shape)
...
class _NamespaceCollectionFake:
    """duck-typed :class:`NamespaceCollection` keyed on the conversation triple."""
...
class _NamespaceCollectionRaisingFake(_NamespaceCollectionFake):
    """variant whose ``save_entity`` raises.

    exercises the create-failed denial path in
    :func:`authorize_conversation_access`.
    """
...
def _reader_role() -> Role:
    return Role(
        id=uuid4(),
        name="ConversationReader",
        permissions={"conversation": frozenset({ACTION_CONVERSATION_READ})},
        is_built_in=True,
    )
...
def _build_deps(
    *,
    ...
    """build a :class:`ConversationAuthorizerDependencies` bundle with ACL mocks.

    :param namespace_collection: fake namespace collection
    :ptype namespace_collection: Any
    :param memberships_for_user: user-side memberships
    ...
    :rtype: ConversationAuthorizerDependencies
    """
    ...
    class _GrantLoader:
        ...
        async def load_assignments_for_groups(
            ...
            """return all configured assignments.
...
class TestAuthorizeConversationAccess:
    ...
    async def test_owner_role_grants_delete(self) -> None:
        agent_id = uuid4()
        customer_id = uuid4()
        user_id = uuid4()
        group_id = uuid4()
        ...
        result = await authorize_conversation_access(
            action=ACTION_CONVERSATION_DELETE,
            agent_id=agent_id,
            customer_id=customer_id,
            caller_user_id=user_id,
            caller_agent_id=None,
