        ...
        async with self._bucket_lock:
            if self._bucket is None:
                self._bucket = await self._client.kv_bucket(
                    ...
                    ttl=self._ttl,
                    # file storage, temporarily: a claim must survive a NATS restart for the whole
                    # ttl window. with memory storage a restart wipes the bucket and the self-heal
                    # recreates it EMPTY, so a caller retrying an already-"completed"/"failed"
                    # operation would get status="claimed" again and redo the work. the durable
                    # answer is L3 through a BaseCollection claim, not file-backed L2; see
                    # docs/design-durable-coordination.md.
                    storage="file",
