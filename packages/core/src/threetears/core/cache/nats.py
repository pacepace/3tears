...
class NatsClient:
    ...
    async def get(self, bucket: str, key: str) -> bytes | None:
        ...
        if kv is None:
            _logger.warning("get called for unknown bucket", extra={"extra_data": {"bucket": bucket, "key": key}})
            return None
        try:
            ...
            _logger.warning("KV get failed", extra={"extra_data": {"bucket": bucket, "key": key, "error": str(exc)}})
            return None
    ...
    async def put(self, bucket: str, key: str, value: bytes) -> bool:
        ...
        kv = self._buckets.get(bucket)
        if kv is None:
            _logger.warning("put called for unknown bucket", extra={"extra_data": {"bucket": bucket, "key": key}})
            return False
        try:
            await kv.put(key, value)
            return True
        except Exception as exc:
            _logger.warning("KV put failed", extra={"extra_data": {"bucket": bucket, "key": key, "error": str(exc)}})
            return False
    ...
    async def delete(self, bucket: str, key: str) -> bool:
        ...
        kv = self._buckets.get(bucket)
        if kv is None:
            _logger.warning("delete called for unknown bucket", extra={"extra_data": {"bucket": bucket, "key": key}})
            return False
        try:
            await kv.delete(key)
            return True
            ...
            _logger.warning("KV delete failed", extra={"extra_data": {"bucket": bucket, "key": key, "error": str(exc)}})
            return False
    ...
    async def create(self, bucket: str, key: str, value: bytes) -> bool:
        ...
        kv = self._buckets.get(bucket)
        if kv is None:
            _logger.warning("create called for unknown bucket", extra={"extra_data": {"bucket": bucket, "key": key}})
            return False
        try:
            ...
            _logger.warning("KV create failed", extra={"extra_data": {"bucket": bucket, "key": key, "error": str(exc)}})
            return False
    ...
    async def update(self, bucket: str, key: str, value: bytes, revision: int) -> int | None:
        ...
        kv = self._buckets.get(bucket)
        if kv is None:
            _logger.warning("update called for unknown bucket", extra={"extra_data": {"bucket": bucket, "key": key}})
            return None
        try:
            ...
            _logger.warning("KV update failed", extra={"extra_data": {"bucket": bucket, "key": key, "error": str(exc)}})
            return None
    ...
    async def get_entry(self, bucket: str, key: str) -> tuple[bytes, int] | None:
        ...
        kv = self._buckets.get(bucket)
        if kv is None:
            _logger.warning("get_entry called for unknown bucket", extra={"extra_data": {"bucket": bucket, "key": key}})
            return None
        try:
            entry = await kv.get(key)
